<div class="alert alert-success" role="alert">
  <h4 class="alert-heading">¡Bienvenido al Insurance Tech de Grupo Nobe!</h4>
  <p>Desde su panel de control podrá ver la infomación necesaria manejar sus pólizas y cotizaciones.</p>
</div>
<div class="card-deck">
  <div class="card">
    <div class="card-header text-white bg-primary">
      <h1><?= $resultado["total"] ?></h1>
      <br>
      <h5>Registros <br> Totales</h5>
    </div>
    <div class="card-body">
      <a href="<?= constant('url') ?>cotizaciones/buscar" class="card-link">Ver más</a>
    </div>
  </div>
  <div class="card">
    <div class="card-header text-white bg-success">
      <h1><?= $resultado["pendientes"] ?></h1>
      <br>
      <h5>Cotizaciones <br> al Mes</h5>
    </div>
    <div class="card-body">
      <a href="<?= ($resultado["pendientes"] > 0) ? constant('url') . "cotizaciones/buscar/" . $resultado["filtro_pendientes"] : "#"; ?>" class="card-link">Ver más</a>
    </div>
  </div>
  <div class="card">
    <div class="card-header text-white bg-info">
      <h1><?= $resultado["emisiones"] ?></h1>
      <br>
      <h5>Emisiones <br> al Mes</h5>
    </div>
    <div class="card-body">
      <a href="<?= ($resultado["emisiones"] > 0) ? constant('url') . "cotizaciones/buscar/" . $resultado["filtro_emisiones"] : "#"; ?>" class="card-link">Ver más</a>
    </div>
  </div>
  <div class="card">
    <div class="card-header text-white bg-warning">
      <h1><?= $resultado["vencimientos"] ?></h1>
      <br>
      <h5>Vencimientos <br> al Mes</h5>
    </div>
    <div class="card-body">
      <a href="<?= ($resultado["vencimientos"] > 0) ? constant('url') . "cotizaciones/buscar/" . $resultado["filtro_vencimientos"] : "#"; ?>" class="card-link">Ver más</a>
    </div>
  </div>
</div>