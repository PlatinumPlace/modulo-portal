...
$trato = $tratos->getRecord("Deals", $id);
...
if ($_POST) {
    ...
    foreach ($_FILES["documentos"]["error"] as $key => $error) {
        if ($error == UPLOAD_ERR_OK) {
            ...
            $tratos->uploadAttachment("Deals", $id, "$ruta/$name");
            unlink("$ruta/$name");
        }
    }
...
<table class="table">
    ...
    <tbody>
        <?php
        $adjuntos = $tratos->getAttachments("Deals", $id, $num_pag, 10);
...
<form enctype="multipart/form-data" method="POST" action="?pagina=adjuntar?id=<?= $id ?>">
    ...
    |
    <a href="?pagina=detalles<?= $trato->getFieldValue("Type") . "_2&id=" . $id ?>" class="btn btn-info">Cancelar</a>
