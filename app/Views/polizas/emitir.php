...
<h1 class="mt-4">Emitir</h1>
<hr>

<form action="<?= site_url("polizas/" . strtolower($cotizacion->getFieldValue('Tipo'))) ?>" method="post" enctype="multipart/form-data">
    <?= csrf_field() ?>

    <input value="<?= $cotizacion->getEntityId() ?>" name="id" hidden />
    <input value="<?= $cotizacion->getFieldValue('Plan') ?>" name="plantipo" hidden />
    <input value="<?= $cotizacion->getFieldValue('Suma_asegurada') ?>" name="suma" hidden />
    ...
    <?php if ($cotizacion->getFieldValue('Tipo') == "Auto") : ?>
        <?= $this->include('auto/emitir') ?>
    <?php endif ?>

    <?php if ($cotizacion->getFieldValue('Tipo') == "Vida") : ?>
        <?= $this->include('vida/emitir') ?>
    <?php endif ?>
