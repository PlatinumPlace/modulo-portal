...
class cotizacion
{
    ...
    public function contratoAdjuntos($plan_id)
    {
        ...
        foreach ($contratos as $contrato) {
            if ($plan->getFieldValue('Vendor_Name')->getEntityId() == $contrato->getFieldValue('Aseguradora')->getEntityId()) {
                $result["adjuntos"] = $this->api->getAttachments("Contratos", $contrato->getEntityId());
                $result["contrato_id"] = $contrato->getEntityId();
                return $result;
            }
        }
    }

    public function descargarAdjuntoContrato()
    {
        $documento = $this->api->downloadAttachment("Contratos", $_GET["contract_id"], $_GET["attachment_id"]);
