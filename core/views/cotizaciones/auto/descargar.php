...
<html lang="en">
    ...
    <body>
                    ...
                    <div class="row">
                        ...
                        <?php foreach ($planes as $plan) : ?>
                            <?php if ($plan->getNetTotal() > 0) : ?>
                                <div class="col-2">
                                    <?php
                                    $imagen_aseguradora = $cotizaciones->imagenAseguradora($plan->getProduct()->getEntityId());
                                    $coberturas = $cotizaciones->coberturas($plan->getProduct()->getEntityId());
                                    ?>
                                    ...
                                    <div class="card border-0">
                                        <div class="card-body">
                                            ...
                                            <p class="card-text">
                                                <small>
                                                    <?php
                                                    $riesgo_compresivo = $detalles->getFieldValue('Valor_Asegurado') * ($coberturas->getFieldValue('Riesgos_comprensivos') / 100);
                                                    ?>
                                                    ...
                                                    <?php
                                                    $colision = $detalles->getFieldValue('Valor_Asegurado') * ($coberturas->getFieldValue('Colisi_n_y_vuelco') / 100);
                                                    ?>
                                                    RD$<?= number_format($colision) ?> <br>
                                                    <?php
                                                    $incendio = $detalles->getFieldValue('Valor_Asegurado') * ($coberturas->getFieldValue('Incendio_y_robo') / 100);
                                                    ?>
        ...
        <script>
            ...
            setTimeout(function () {
                window.print();
                window.location = url + "cotizaciones/detallesAuto?id=" + id;
            }, time);
        </script>
