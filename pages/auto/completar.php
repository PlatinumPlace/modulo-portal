...
$clientes = $cotizaciones->lista_clientes($_SESSION["usuario"]["id"]);

if ($_POST) {

    if (!empty($_POST["mis_clientes"])) {
        $alerta =  $cotizaciones->completar_auto_cliente_existente($id, $_POST["mis_clientes"]);
    } else {
        $alerta =  $cotizaciones->completar_auto_cliente_nuevo($id);
    }

    if (!$alerta) {
        $alerta = "Cliente agregado";
        header("Location:" . constant("url") . "auto/detalles/$id/$alerta");
        exit;
    }
}
