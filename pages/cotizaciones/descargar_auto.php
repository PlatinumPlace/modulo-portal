...
$detalles = $api->buscar_registro_por_criterio("Quotes", $criterio);
...
if (in_array($cotizacion->getFieldValue("Stage"), $emitida)) {
    ...
    $aseguradora = $api->obtener_registro("Vendors", $cotizacion->getFieldValue('Aseguradora')->getEntityId());
    $criterio = "Deal_Name:equals:" . $datos;
    $a = $api->buscar_registro_por_criterio("Quotes", $criterio);
    foreach ($a as $c) {
        $detalles = $c;
    }
    $imagen_aseguradora = $api->descargar_foto("Vendors", $detalles->getFieldValue('Aseguradora')->getEntityId(), "img/");
    $contrato = $api->obtener_registro("Contratos", $detalles->getFieldValue('Contrato')->getEntityId());
}
...
<body>
    <div class="container">
        <div class="row">
            ...
            <div class="col-2">
                <?php if (in_array($cotizacion->getFieldValue("Stage"), $emitida)) : ?>
                    <b> Póliza No.</b> <br> <?= $detalles->getFieldValue('P_liza')->getLookupLabel() ?>
                    <br>
                    <b> cotizacion No.</b>
                <?php else : ?>
                    <br>
                    ...
                    <?php foreach ($detalles as $resumen) : ?>
                        <div class="col-2">
                            <?php if ($resumen->getFieldValue('Grand_Total') > 0) : ?>
                                <?php $contrato = $api->obtener_registro("Contratos", $resumen->getFieldValue('Contrato')->getEntityId()) ?>
                                <h5>
                                    <?php if (!in_array($cotizacion->getFieldValue("Stage"), $emitida)) : ?>
                                        <?php $imagen_aseguradora = $api->descargar_foto("Vendors", $resumen->getFieldValue('Aseguradora')->getEntityId(), "img/") ?>
                                ...
                                <p class="card-text">
                                    <?php
                                    $resultado = $cotizacion->getFieldValue('Valor_Asegurado') * ($contrato->getFieldValue('Riesgos_comprensivos') / 100);
                                    ...
                                    <?php
                                    $resultado = $cotizacion->getFieldValue('Valor_Asegurado') * ($contrato->getFieldValue('Colisi_n_y_vuelco') / 100);
                                    echo "RD$" . number_format($resultado);
                                    ?><br>
                                    <?php
                                    $resultado = $cotizacion->getFieldValue('Valor_Asegurado') * ($contrato->getFieldValue('Incendio_y_robo') / 100);
                                    echo "RD$" . number_format($resultado);
                                    ?><br><br>
                                    <b>&nbsp;</b><br>
        ...
        <?php if (in_array($cotizacion->getFieldValue("Stage"), $emitida)) : ?>
                ...
                <div class="col-6 border">
                    ...
                    <div class="row">
                        ...
                        <div class="col-4">
                            ...
                            <br>
                            <?= $cotizacion->getFieldValue('Fecha_de_emisi_n') ?>
                            <br>
            ...
            <div class="row">
                ...
                <div class="col-3">
                    <b>Póliza No. <br> <?= $detalles->getFieldValue('P_liza')->getLookupLabel() ?></b>
                </div>
            </div>
            <ol>
                <li>Este certificado es sólo un cotizacion de las condiciones principales contenidas en la póliza.</li>
    ...
    <input value="<?= $datos ?>" id="id" hidden>
